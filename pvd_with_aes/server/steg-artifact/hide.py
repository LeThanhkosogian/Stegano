...
import numpy as np
from PIL import Image
from encryptAES import encrypt_message
...
if __name__ == "__main__":
    ...
    #image_path = "C:\\Users\\Admin\\Desktop\\Stegano\\temp\\pvd_with_aes\\server\\normal.png"  # Path to the input image
    #output_path = "C:\\Users\\Admin\\Desktop\\Stegano\\temp\\pvd_with_aes\\server\\steg.png"  # Path to save the encoded image

    image_path = input("Nhập đường dẫn đến ảnh: ")
    output_path = input("Nhập đường dẫn để lưu ảnh chứa thông điệp đã mã hóa: ")
    
    hide_message_into_image(image_path, encrypted_message, output_path)
    print(f"Message was hidden in: {output_path}")
